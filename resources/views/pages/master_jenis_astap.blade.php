<x-layout title="Jenis ASTAP Kode 108 BMD - SIMAT-RK">
    ...
    <div x-data="{
        ...
        @if (session('success'))
            <div class="mb-4 p-4 rounded-2xl bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-xs font-bold flex items-center justify-between">
                        ...
                        @forelse ($kode108List as $index => $item)
                            <tr class="hover:bg-slate-800/40 transition-colors">
                                ...
                                <td class="px-3 py-4 text-center space-x-1 whitespace-nowrap">
                                    ...
                                    <form action="{{ route('master.jenis_astap.destroy', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data Kode 108 ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
