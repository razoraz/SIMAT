    ...
    <div class="flex flex-col gap-4">
        ...
        <div class="flex items-center gap-2 flex-wrap text-xs bg-slate-950/60 p-2 rounded-2xl border border-slate-800/80">
            ...
            <button type="button" @click="statusFilter = 'selesai'"
                ...
                class="px-3.5 py-1.5 rounded-xl border text-xs font-bold transition-all flex items-center space-x-1.5 cursor-pointer shrink-0 active:scale-95">
                <span>✓ Disahkan Bupati / Selesai</span>
                <span class="px-1.5 py-0.2 text-[10px] font-mono font-black rounded-md"
            ...
            {{-- Button Peminjaman Aktif --}}
            <button type="button" @click="statusFilter = 'pinjam_aktif'"
                :class="statusFilter === 'pinjam_aktif' 
                ...
                class="px-3.5 py-1.5 rounded-xl border text-xs font-bold transition-all flex items-center space-x-1.5 cursor-pointer shrink-0 active:scale-95">
                <span>⏱️ Peminjaman Aktif</span>
                <span class="px-1.5 py-0.2 text-[10px] font-mono font-black rounded-md"
                    :class="statusFilter === 'pinjam_aktif' ? 'bg-slate-950/40 text-slate-950' : 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20'"
                    x-text="countPinjamAktif"></span>
            </button>
        ...
        <!-- Filter Jenis Transaksi & Search Input Row -->
        <div class="flex flex-col sm:flex-row items-center gap-3 w-full pt-2 border-t border-slate-800/80">
            ...
            <div class="w-full sm:w-56 shrink-0">
                <select x-model="jenisFilter"
                    ...
                    <option value="Transfer Antar-OPD">🏛️ Transfer Antar-OPD (Alih Status)</option>
                    <option value="Peminjaman Antar-OPD">🤝 Peminjaman Antar-OPD (Pinjam Pakai)</option>
                    <option value="Penyerahan ke BPKAD">📦 Penyerahan ke BPKAD (Pengelola BMD)</option>
                </select>
            </div>

            <!-- Search Input -->
            <div class="relative flex-1 w-full">
                <input type="text" x-model="searchQuery" placeholder="Cari nomor BAST / nama OPD tujuan / nama aset / NIBAR / dasar SK..."
            ...
            <div class="flex items-center space-x-2 shrink-0">
                <span class="px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-[11px] font-semibold text-slate-300">
                    Menampilkan <span class="text-indigo-400 font-bold" x-text="filteredMutasis.length"></span> dari <span class="text-white font-bold" x-text="mutasiEksternals.length"></span> Transaksi
                </span>
