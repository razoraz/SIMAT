<!-- ========================================================================= -->
<!-- LANGKAH 2: FILTERING BERTINGKAT KLASIFIKASI KODE BARANG 108 (HIBAH)       -->
<!-- ========================================================================= -->
<div x-show="currentStep === 2" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-6">
    <div>
        ...
        <h2 class="text-lg font-bold text-white flex items-center space-x-2">
            ...
            <span>Langkah 2: Memilih Klasifikasi Jenis ASTAP &amp; Kode Barang 108</span>
        </h2>
        <p class="text-xs text-slate-400 mt-1">Pilih Jenis Aset PMDN 108 ➔ Sub Rincian Objek ➔ Identitas Spesifik Barang ➔ Kategori Pencatatan secara berjenjang:</p>
    </div>

    <!-- Filter Berjenjang PMDN 108 (Tanpa Rekening Belanja APBD & Tanpa Pagu Anggaran) -->
    <div class="p-6 rounded-3xl bg-slate-950/80 border border-cyan-500/30 space-y-5 shadow-2xl">
        
        <!-- Tingkat 1: JENIS ASET PMDN 108 Card Filter Model -->
        <div class="space-y-2 relative" @click.away="isJenis108Open = false">
            <div class="flex items-center justify-between">
                <label class="block text-slate-200 font-bold text-xs flex items-center space-x-2">
                    <span class="w-5 h-5 rounded-full bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 text-[10px] font-black flex items-center justify-center">1</span>
        ...
        <!-- Tingkat 2: SUB RINCIAN OBJEK PMDN 108 Card Filter Model -->
        <div class="space-y-2 relative" @click.away="isSubRincian108Open = false">
            ...
            <p class="text-[10.5px] text-slate-400 italic">
                💡 <strong class="text-emerald-300">Catatan:</strong> Bagian ini boleh dikosongkan jika Anda tidak hafal kode sub-rincian 108. Saat memilih <strong>Identitas Barang</strong> pada nomor 3 di bawah, Sub Rincian 108 ini akan otomatis terisi.
            </p>
                ...
                <div @click="formData.sub_rincian_kode = ''; formData.sub_rincian_nama = ''; searchSubRincian108 = ''; isSubRincian108Open = false"
                    ...
                    <div class="min-w-0 pr-3 flex items-center space-x-2">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-amber-400/80">Biarkan kosong, akan otomatis terisi saat memilih Identitas Barang di nomor 3</p>
                        </div>
                    </div>
        ...
        <!-- Tingkat 3: IDENTITAS BARANG PMDN 108 Card Filter Model -->
        <div class="space-y-2 relative" @click.away="isNamaBarang108Open = false">
            <div class="flex items-center justify-between">
                <label class="block text-slate-200 font-bold text-xs flex items-center space-x-2">
                    ...
                    <span class="text-[10px] text-purple-400 font-normal italic">(Nama & Kode Barang 108 Spesifik)</span>
                    <span class="text-rose-400">*</span>
                </label>
        ...
        <!-- Tingkat 4: PILIHAN KATEGORI EKSTRAKOMTABEL (HANYA UNTUK KIB B & KIB E) -->
        <div x-show="formData.jenis_aset_kode" x-transition class="space-y-2">
            <div class="flex items-center space-x-2 mb-1">
                <span class="w-5 h-5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/40 text-[10px] font-black flex items-center justify-center">4</span>
            ...
            <div x-show="isMesin || isAsetLainnya" class="p-4 rounded-2xl bg-slate-950/80 border border-amber-500/30 space-y-3 shadow-xl">
                ...
                <div class="flex items-start space-x-2 p-2.5 rounded-xl bg-amber-950/30 border border-amber-500/20">
                    ...
                    <p class="text-[10.5px] text-amber-300/90 leading-relaxed">
                        ...
                        Barang hibah dengan taksiran harga satuan <strong class="text-emerald-300">&gt; Rp 300.000</strong> dicatat sebagai <strong>Aset Reguler (KIB)</strong>.
                        Barang hibah dengan taksiran harga satuan <strong class="text-cyan-300">≤ Rp 300.000</strong> dicatat sebagai <strong>Barang Ekstrakomtabel (Extracom)</strong>.
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div @click="formData.is_extracomtable = false"
                        ...
                        <p class="text-[11px] text-slate-300 leading-relaxed">
                            Aset kapitalisasi standar dengan taksiran harga satuan <strong class="text-emerald-300">&gt; Rp 300.000 / unit</strong>. Dicatat dalam Neraca Aset Tetap KIB.
                        </p>
                    </div>
                    ...
                    <div @click="formData.is_extracomtable = true"
                        ...
                        <p class="text-[11px] text-slate-300 leading-relaxed">
                            Barang non-kapitalisasi dengan harga satuan <strong class="text-cyan-300">≤ Rp 300.000 / unit</strong>. Dicatat di Daftar Ekstrakomtabel RSUD terpisah.
                        </p>
                    </div>
                </div>
        ...
        <!-- Tingkat 5: PERIODE PEMBUKUAN ASET HIBAH (Tahun & Triwulan - Dipindahkan ke Langkah 2) -->
        <div class="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                <label class="block text-slate-300 font-semibold text-xs mb-1 flex items-center justify-between">
                    <span>📅 TAHUN PEMBUKUAN HIBAH</span>
                    <span class="text-[10px] text-cyan-400 font-mono">1990 - 2100</span>
                </label>
                <div class="relative">
                    <input type="number" 
                           min="1990" 
    ...
    <!-- ========================================================================= -->
    <!-- LIVE PREVIEW TABEL ASET HIBAH (FORMAT RESMI PERMENDAGRI 108)               -->
    <!-- ========================================================================= -->
