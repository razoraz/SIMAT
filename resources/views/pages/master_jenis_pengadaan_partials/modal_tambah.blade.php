...
<div x-show="showAddModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/85 backdrop-blur-md p-4" x-cloak>
    <div @click.away="showAddModal = false" class="bg-slate-900 border border-slate-800 rounded-3xl max-w-3xl w-full p-6 shadow-2xl space-y-4 relative">
        <!-- Tombol Close Corner -->
            ...
            <div class="p-3.5 rounded-2xl bg-emerald-950/20 border border-emerald-500/30 space-y-2">
                ...
                <div class="grid grid-cols-3 gap-2">
                    <div>
                        ...
                        <input type="text" 
                               name="program_kode" 
                               x-model="newFormData.program_kode" 
                               @input="onKodeProgramInputNew()" 
                               placeholder="0.00.01" 
                               required 
                    ...
                    <div class="col-span-2 relative" @click.away="showProgramDropdownNew = false">
                        ...
                        <div class="relative">
                            <input type="text" 
                                   name="program_nama" 
                                   x-model="newFormData.program_nama" 
                                   @focus="showProgramDropdownNew = true" 
                                   @input="showProgramDropdownNew = true" 
                                   @keydown.escape="showProgramDropdownNew = false" 
                                   required 
                                   autocomplete="off"
                        ...
                        <div x-show="showProgramDropdownNew" 
                            ...
                            <div class="px-3 py-1.5 bg-slate-950/90 text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center justify-between border-b border-slate-800">
                                ...
                                <span class="text-emerald-400 font-mono text-[9px]" x-text="totalProgramCountNew + ' tersedia'"></span>
                            </div>

                            <template x-for="item in filteredProgramListNew" :key="item.program_kode + item.program_nama">
                                <div @click="selectProgramNew(item)" 
                                     class="px-3 py-2 hover:bg-emerald-500/15 cursor-pointer transition-colors group flex items-center justify-between gap-2"
                                     :class="{'bg-emerald-500/10': newFormData.program_kode === item.program_kode}">
                            ...
                            <template x-if="filteredProgramListNew.length === 0">
                                <div class="p-3 text-center text-xs text-slate-400">
                                    <p class="text-emerald-400 font-semibold text-[11px]">✨ Program Pengadaan Baru</p>
                                    <p class="text-[10px] text-slate-400 mt-0.5">Gunakan nama ini & silakan tentukan Kode Program di samping</p>
                                </div>
                            </template>
                        </div>
                    </div>
            ...
            <div class="p-3.5 rounded-2xl bg-amber-950/20 border border-amber-500/30 space-y-2">
                ...
                <div class="grid grid-cols-3 gap-2">
                    <div>
                        ...
                        <input type="text" 
                               name="kegiatan_kode" 
                               x-model="newFormData.kegiatan_kode" 
                               @input="onKodeKegiatanInputNew()" 
                    ...
                    <div class="col-span-2 relative" @click.away="showKegiatanDropdownNew = false">
                        <label class="block text-slate-400 mb-1 flex items-center justify-between">
                        ...
                        <div class="relative">
                            <input type="text" 
                                   name="kegiatan_nama" 
                                   x-model="newFormData.kegiatan_nama" 
                                   @focus="showKegiatanDropdownNew = true" 
                                   @input="showKegiatanDropdownNew = true" 
                                   @keydown.escape="showKegiatanDropdownNew = false" 
                                   required 
                                   autocomplete="off"
                        ...
                        <div x-show="showKegiatanDropdownNew" 
                             x-transition.opacity.duration.150ms 
                             class="absolute left-0 right-0 z-50 mt-1 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden max-h-48 overflow-y-auto divide-y divide-slate-800" 
                             style="display: none;">
                            <div class="px-3 py-1.5 bg-slate-950/90 text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center justify-between border-b border-slate-800">
                                <span>Pilih Kegiatan Pengadaan</span>
                                <span class="text-amber-400 font-mono text-[9px]" x-text="totalKegiatanCountNew + ' tersedia'"></span>
                            </div>

                            <template x-for="item in filteredKegiatanListNew" :key="item.kegiatan_kode + item.kegiatan_nama">
                                <div @click="selectKegiatanNew(item)" 
                                     class="px-3 py-2 hover:bg-amber-500/15 cursor-pointer transition-colors group flex items-center justify-between gap-2"
                                     :class="{'bg-amber-500/10': newFormData.kegiatan_kode === item.kegiatan_kode}">
                                    <div class="flex items-center space-x-2 min-w-0">
                            ...
                            <template x-if="filteredKegiatanListNew.length === 0">
                                <div class="p-3 text-center text-xs text-slate-400">
                                    <p class="text-amber-400 font-semibold text-[11px]">✨ Kegiatan Baru</p>
                                    <p class="text-[10px] text-slate-400 mt-0.5">Gunakan nama ini & silakan tentukan Kode Kegiatan di samping</p>
                                </div>
                            </template>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-3 gap-2">
                    <div>
                        ...
                        <input type="text" 
                               ...
                               x-model="newFormData.sub_kegiatan_kode" 
                    ...
                    <div class="col-span-2">
                        ...
                        <input type="text" 
                               ...
                               x-model="newFormData.sub_kegiatan_nama" 
