        ...
        <form @submit.prevent="submitForm" class="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
            ...
            <div x-show="step === 1" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-6">
                ...
                <div class="p-5 rounded-2xl bg-slate-950/70 border border-indigo-500/30 space-y-4 shadow-xl">
                    ...
                    <!-- Toko / Supplier Rekanan (Combobox / Filter Riwayat & Bebas Ketik) -->
                    <div class="relative space-y-1.5" @click.away="isTokoDropdownOpen = false">
                        ...
                        <div class="relative">
                            <input type="text" 
                                ...
                                required
                                autocomplete="off"
                                placeholder="Ketik atau pilih nama toko / rekanan (contoh: Toko Barokah Medika, UD. Mandiri...)"
                                class="w-full bg-slate-900 border border-slate-700 focus:border-indigo-400 rounded-xl px-4 py-2.5 pl-10 pr-10 text-xs text-white placeholder-slate-500 focus:outline-none font-bold transition-all shadow-inner">
                            
                            <!-- Ikon Toko -->
                            <svg class="w-4 h-4 text-indigo-400 absolute left-3.5 top-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div x-show="isTokoDropdownOpen" 
                            ...
                            <!-- Notifikasi jika mengetik toko baru -->
                            <template x-if="formData.rekening_penyedia && filteredTokoList.length === 0">
                                <div class="p-3 text-center text-xs text-slate-400 bg-slate-950/50">
                                    <span class="text-indigo-300 font-semibold" x-text="'➕ Gunakan Toko Baru: &quot;' + formData.rekening_penyedia + '&quot;'"></span>
                                    <p class="text-[10px] text-slate-500 mt-0.5">Nama toko ini akan otomatis tersimpan ke riwayat setelah formulir disimpan.</p>
                                </div>
                            </template>
                        </div>
                        
                        <!-- Rekomendasi Cepat Toko (Badge Shortcut) -->
                        <div class="pt-1 flex flex-wrap items-center gap-1.5">
                            ...
                            <button type="button" @click="selectToko('Toko Barokah Medika')"
                            ...
                            <button type="button" @click="selectToko('UD. Mandiri Elektrik')"
                                class="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] border border-slate-700 transition cursor-pointer">
                                UD. Mandiri Elektrik
                            </button>
                            <button type="button" @click="selectToko('Apotek & Alkes Sehat Bersama')"
                                class="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] border border-slate-700 transition cursor-pointer">
                                Apotek &amp; Alkes Sehat
                            </button>
                            <button type="button" @click="selectToko('CV. Sumber Makmur Perkasa')"
                                class="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] border border-slate-700 transition cursor-pointer">
                                CV. Sumber Makmur
                            </button>
                        </div>
    ...
    <script>
        function formRekening() {
            return {
                ...
                masterTokoList: @json(!empty($dbTokoPenyedias) ? $dbTokoPenyedias : [
                    'Toko Barokah Medika',
                    'UD. Mandiri Elektrik',
                    'Apotek & Alkes Sehat Bersama',
                    'CV. Sumber Makmur Perkasa'
                ]),
                ...
                get filteredTokoList() {
                    ...
                    if (!q) {
                        return this.masterTokoList.slice(0, 15);
                    }
                    return this.masterTokoList.filter(t => t && t.toLowerCase().includes(q));
                },
