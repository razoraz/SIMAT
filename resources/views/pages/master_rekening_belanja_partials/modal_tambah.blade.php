...
<div x-show="showAddModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/85 backdrop-blur-md p-4" x-cloak>
    <div @click.away="showAddModal = false" class="bg-slate-900 border border-slate-800 rounded-3xl max-w-3xl w-full p-6 shadow-2xl space-y-4 relative">
        ...
        <form method="POST" action="{{ route('master.rekening_belanja.store') }}" class="space-y-3.5 text-xs" @submit="confirmAddForm($event)">
            @csrf
            ...
            <div class="p-3.5 rounded-2xl bg-blue-950/20 border border-blue-500/30 space-y-2">
                ...
                <div class="grid grid-cols-3 gap-2">
                    <div>
                        ...
                        <input type="text" 
                               name="kelompok" 
                               x-model="newFormData.kelompok" 
                               @input="onKodeKelompokInputNew()" 
                               placeholder="5.2.02" 
                               required 
                    ...
                    <div class="col-span-2 relative" @click.away="showKelompokDropdownNew = false">
                        <label class="block text-slate-400 mb-1 flex items-center justify-between">
                            ...
                            <span class="text-[10px] text-blue-400 font-normal">Ketik untuk filter / pilih kelompok</span>
                        </label>
                        <div class="relative">
                            <input type="text" 
                                   name="nama_kelompok" 
                                   x-model="newFormData.nama_kelompok" 
                                   @focus="showKelompokDropdownNew = true" 
                                   @input="showKelompokDropdownNew = true" 
                                   @keydown.escape="showKelompokDropdownNew = false" 
                                   required 
                                   autocomplete="off"
                        ...
                        <div x-show="showKelompokDropdownNew" 
                            ...
                            <div class="px-3 py-1.5 bg-slate-950/90 text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center justify-between border-b border-slate-800">
                                ...
                                <span class="text-blue-400 font-mono text-[9px]" x-text="totalKelompokCountNew + ' tersedia'"></span>
                            </div>

                            <template x-for="item in filteredKelompokListNew" :key="item.kelompok + item.nama_kelompok">
                                <div @click="selectKelompokNew(item)" 
                                     class="px-3 py-2 hover:bg-blue-500/15 cursor-pointer transition-colors group flex items-center justify-between gap-2"
                                     :class="{'bg-blue-500/10': newFormData.kelompok === item.kelompok}">
                            ...
                            <template x-if="filteredKelompokListNew.length === 0">
                                <div class="p-3 text-center text-xs text-slate-400">
                                    <p class="text-blue-400 font-semibold text-[11px]">✨ Kelompok Belanja Kustom</p>
                                    <p class="text-[10px] text-slate-400 mt-0.5">Gunakan nama ini & silakan tentukan Kode Kelompok di samping</p>
                                </div>
                            </template>
                        </div>
                    </div>
            ...
            <div class="p-3.5 rounded-2xl bg-cyan-950/20 border border-cyan-500/30 space-y-2">
                ...
                <div class="grid grid-cols-3 gap-2">
                    ...
                    <div class="col-span-2">
                        ...
                        <input type="text" 
                               name="nama_belanja" 
                               x-model="newFormData.nama_belanja" 
