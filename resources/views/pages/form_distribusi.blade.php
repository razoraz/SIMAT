            ...
            return {
                ...
                async submitForm() {
                    if (!this.formData.tujuan || this.formData.tujuan.trim() === '') {
                        this.showSimatToast('⚠️ Silakan pilih Tujuan Unit / Paviliun penerima barang!', 'warning');
                        return;
                    }

                    if (!this.formData.items || this.formData.items.length === 0) {
                        this.showSimatToast('⚠️ Minimal harus ada 1 barang dalam transaksi distribusi!', 'warning');
                        return;
                    }

                    const emptyItem = this.formData.items.find(it => !it.nama_barang || it.nama_barang.trim() === '');
                    if (emptyItem) {
                        this.showSimatToast('⚠️ Ada baris barang yang belum dipilih/diisi nama barangnya. Silakan lengkapi data barang!', 'warning');
                        return;
                    }
