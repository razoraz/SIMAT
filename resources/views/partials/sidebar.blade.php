...
<aside x-show="sidebarOpen" x-transition:enter="transition ease-out duration-300 transform"
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-6 scrollbar-thin scrollbar-thumb-slate-800">
            ...
            <div>
                ...
                <div class="space-y-1">
                    ...
                    @if ($role === 'sub_admin' || $user->canAccess('mutasi'))
                        @if ($role === 'sub_admin')
                            <!-- Jika Sub Admin: langsung ke Mutasi Internal (tidak ada akses Mutasi Eksternal) -->
                            <a href="{{ route('mutasi.index') }}" @click="if (isMobile) sidebarOpen = false"
                                class="flex items-center space-x-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 {{ request()->routeIs('mutasi.*') ? 'bg-gradient-to-r from-emerald-600/20 to-teal-600/20 text-emerald-400 border border-emerald-500/30 shadow-sm' : 'text-slate-400 hover:text-white hover:bg-slate-800/60' }}">
        ...
        <!-- Section 4: Audit & Pemulihan Sistem (Pusat Data Terhapus) -->
        @if ($role === 'admin' || $role === 'master_admin')
            <div>
                <div class="px-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Audit & Pemulihan</div>
                <div class="space-y-1">
                    <a href="{{ route('recycle_bin.index') }}" @click="if (isMobile) sidebarOpen = false"
                        class="flex items-center space-x-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 {{ request()->routeIs('recycle_bin.*') ? 'bg-gradient-to-r from-emerald-600/20 to-teal-600/20 text-emerald-400 border border-emerald-500/30 shadow-sm' : 'text-slate-400 hover:text-white hover:bg-slate-800/60' }}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
