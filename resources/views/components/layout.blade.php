    ...
    <div x-data="{
            ...
            openConfirm(detail) {
                this.confirmData = {
                    ...
                    btnText: detail.isBlocked ? null : (detail.btnText !== undefined ? detail.btnText : (detail.type === 'danger' ? 'Hapus Data' : (detail.type === 'warning' ? 'Simpan Perubahan' : 'Tambahkan'))),
                    isBlocked: Boolean(detail.isBlocked),
                    actionUrl: detail.actionUrl || null,
                    actionText: detail.actionText || 'Ajukan Mutasi Aset',
        ...
        <div x-show="showConfirm" x-cloak class="fixed inset-0 z-[999999] flex items-center justify-center bg-slate-950/85 backdrop-blur-md p-4">
            <div @click.away="showConfirm = false"
                ...
                <div class="flex items-start space-x-3.5 pt-1">
                    <div class="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 border border-slate-700/80 shadow-inner"
                         :class="{
                ...
                <template x-if="confirmData.itemName">
                    <div class="p-3.5 bg-slate-950/80 rounded-2xl border border-slate-800/90 space-y-1.5">
                        <span class="text-[10px] uppercase font-mono tracking-wider font-extrabold text-slate-400 block">ITEM TARGET:</span>
